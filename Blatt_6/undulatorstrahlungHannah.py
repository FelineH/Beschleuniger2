...
def r_scal(K, periods):
    ...
    rp = np.array((d * np.sin(ϑ) * np.cos(φ), d * np.sin(ϑ) * np.sin(φ), d * np.cos(ϑ)))[:,np.newaxis]
    r_vek = re + rp
    r = np.linalg.norm(r_vek, axis=0)
    return r
...
def n_vec(K, periods):
    ...
    rp = np.array((d * np.sin(ϑ) * np.cos(φ), d * np.sin(ϑ) * np.sin(φ), d * np.cos(ϑ)))[:,np.newaxis]
    r_vek = re + rp
    n = r_vek/np.linalg.norm(r_vek, axis=0)
    return n
...
for K in range(1,2):
    for P in range(20,21):
        a = np.cross(n_vec(K, P).T, beta_vec(K, P).T)
        I_real = np.cross(n_vec(K, P).T, np.cross(n_vec(K, P).T, beta_vec(K,P).T))[:,0] * np.cos(- np.outer(ω, (integration_interval(P,K) + r_scal(K, P) / const.c))) 
        ...
        I_im = np.cross(n_vec(K, P).T, np.cross(n_vec(K, P).T, beta_vec(K,P).T))[:,0] * np.sin(- np.outer(ω, (integration_interval(P,K) + r_scal(K, P) / const.c)))
